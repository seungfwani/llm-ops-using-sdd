...
def main():
    parser = argparse.ArgumentParser(description="From-scratch training script")
    ...
    logger.info(f"Starting from-scratch training job: {job_id}")
    ...
    for epoch in range(epochs):
        ...
        loss = 2.0 - (epoch + 1) * 0.09
        record_metric(job_id, "loss", max(0.1, loss))
        record_metric(job_id, "perplexity", max(1.5, 10.0 - (epoch + 1) * 0.4))
